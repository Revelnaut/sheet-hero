...
void SongRenderer::render(const Song& song, float playing_position, sf::RenderTarget& target, sf::RenderStates states) const {
	...
	const int measures_in_song{ static_cast<const int>(time_signature.get_ratio() * song.get_grand_staff().get_whole_note_count()) };

	std::cout << song.get_grand_staff().get_whole_note_count() << std::endl;
	...
	for ( int measure_number = 0; measure_number < measures_in_song; ++measure_number ) {
		if ( new_line ) {
			new_line = false;
		...
		float beat_length{ m_settings.get_whole_note_spacing() * time_signature.get_ratio() / static_cast<float>( time_signature.get_numerator() ) };
	...
	//	if ( draw_position.x + ( m_settings.get_whole_note_spacing() * 2 + m_settings.get_bar_width() ) <= m_bounds.width + m_bounds.left ) {
	//		if ( &measure != &song.get_grand_measures().back() ) { // If not last measure
	//			draw_position.x += m_settings.get_whole_note_spacing() + m_settings.get_bar_width() * 0.2f;
...
void SongRenderer::draw_staff(const Song& song, Clef clef, sf::Vector2f position, float width, sf::RenderTarget& target, sf::RenderStates states) const {
	...
	// Vertical bars
	LineShape vertical_line{ 0.0f, 0.0f, 0.0f, m_settings.get_staff_height() };
	...
	MusicalSymbol clef_symbol{ symbol_factory(MusicalGlyph::ClefG) };
	if ( clef == Clef::Treble ) {
		clef_symbol = symbol_factory(MusicalGlyph::ClefG);
	} else {
		clef_symbol = symbol_factory(MusicalGlyph::ClefF);
	}
	clef_symbol.use_font_baseline(true);
	if ( clef == Clef::Treble ) {
		clef_symbol.setOrigin(0, m_settings.get_line_spacing());
	} else {
		clef_symbol.setOrigin(0, m_settings.get_line_spacing() * 3);
	}
	clef_symbol.setPosition(draw_position);
	target.draw(clef_symbol, states);
	...
	draw_position.x += clef_symbol.get_size().x + m_settings.get_time_signature_spacing();
	...
	if ( clef == Clef::Bass ) {
		draw_position.y += m_settings.get_line_spacing();
	}
...
void SongRenderer::draw_grand_staff(const Song& song, sf::Vector2f position, float width, sf::RenderTarget& target, sf::RenderStates states) const {
	...
	draw_staff(song, Clef::Treble, draw_position, width, target, states);
	
	draw_position.y += m_settings.get_staff_height() + m_settings.get_staff_spacing();

	draw_staff(song, Clef::Bass, draw_position, width, target, states);
}
...
//	float measure_width{ m_settings.get_whole_note_spacing() / song.get_time_signature().get_ratio() };
//
//	sf::Vector2f draw_position{ position };
